...
const ChatContainer = () => {
  ...
  const onInsertChat = (talker, type, position, text) =>
    dispatch(insertChat(talker, type, position, text));
  const onUpdateChat = useCallback(
    (id, text) => dispatch(updateChat(id, text)),
    [dispatch],
  );
  const onDeleteChat = useCallback((id) => dispatch(deleteChat(id)), [
    dispatch,
  ]);
  const onToggleChatEdit = useCallback((id) => dispatch(toggleChatEdit(id)), [
    dispatch,
  ]);
  const onChangeType = useCallback((id) => dispatch(changeType(id)), [
    dispatch,
  ]);
  const onChangeTalker = useCallback((id) => dispatch(changeTalker(id)), [
    dispatch,
  ]);
